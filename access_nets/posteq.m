...
%     save('results/posteq.mat','M','L','apdgain','PreqdBm','ber20dbm','optgain','minP')
    ...
    for k=length(L):-1:1
        for j=length(M):-1:1
            ber(k,j) = pam_access_nets_qsub(30,M(j),L(k),1358,30,'MZM','optimized',2,optgain(j,k),22,'fixed td-sr-le',1e-3,PtxdBm+Poff(j),0.8,2);
        end
    end
...
figure(3)
h0=plot(log10(BERtarget),Preq(:,:,1),'x-');
hold on
set(gca,'colororderindex',1)
h1=plot(log10(BERtarget),Preq(:,:,2),'x--');
set(gca,'colororderindex',1)
h2=plot(log10(BERtarget),Preq(:,:,3),'x:');
legend([h0;h1(1);h2(1)],'OOK 25 km','2DPS 3-PAM','4-PAM', 'b2b','10km')
grid on
